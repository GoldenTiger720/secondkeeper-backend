...
class FireSmokeDetector:
    def __init__(self, camera_id=None, rtsp_url="", video_file="video.mp4", use_rtsp=False):
        ...
        print("[INFO] FireSmokeDetector created, will initialize in worker thread")
    ...
    def _initialize_camera(self):
        ...
        if self.is_test_video:
            ...
            try:
                self.camera, created = Camera.objects.get_or_create(
                    name="TEST VIDEO CAMERA - Fire Detection",
                    stream_url="file://test_video_fire",
                    defaults={
        ...
        if self.camera_id:
            try:
                ...
                print(f"[INFO] FireSmokeDetector linked to camera: {self.camera.name}")
            ...
            try:
                ...
                if created:
                    print("[INFO] Created test camera for FireSmokeDetector")
                else:
                    print("[INFO] Using existing test camera for FireSmokeDetector")
            except Exception as e:
    ...
    def _initialize_components(self):
        ...
        with self._initialization_lock:
            ...
            try:
                print(f"[INFO] Initializing FireSmokeDetector in thread {threading.current_thread().name}")
                ...
                print(f"[INFO] FireSmokeDetector initialized successfully in thread {threading.current_thread().name}")
                self._is_initialized = True
                
            except Exception as e:
                self._initialization_error = str(e)
                print(f"[ERROR] Failed to initialize FireSmokeDetector: {e}")
                import traceback
                traceback.print_exc()
                raise
        ...
        try:
            ...
            if not self._is_initialized:
                if self._initialization_error:
                    print(f"[ERROR] FireSmokeDetector not initialized: {self._initialization_error}")
                return frame
            
            if self.host_input is None or self.lb is None:
                print("[ERROR] FireSmokeDetector not properly initialized")
                return frame
            ...
            if label == self.fire_label and confidence >= 0.9:
                if not self.detection_active:
                    ...
                    print(f"[INFO] Fire detection started with confidence: {confidence:.2f}")
                ...
                if self.detection_active:
                    print(f"[INFO] Fire detection ended")
                    self.detection_active = False
                    self.detection_frames = []
                    self.first_detection_frame = None
                
            # Draw detection info on frame
            color = (0, 0, 255) if label == self.fire_label else (0, 255, 0)
    ...
    def save_clip_on_detection(self, current_time, confidence):
        """Save video clip when fire is detected"""
        # Check cooldowns
        ...
        if not frames_to_save:
            print("[WARNING] No frames to save for fire detection")
            return
        ...
        try:
            ...
            print(f"[ERROR] Failed to save fire detection clip: {e}")
    
    def save_clip(self, thumbnail_frame, frames_to_save, detection_time, confidence=0.98):
        """Save video clip and thumbnail, then create alert"""
        try:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            
            # Save thumbnail
            thumbnail_path = os.path.join(self.output_dir, f"fire_thumbnail_{timestamp}.jpg")
            ...
            # Save video clip
            output_path = os.path.join(self.output_dir, f"fire_{timestamp}.mp4")
            fps = 30
    ...
    def _create_alert(self, video_path, thumbnail_path, confidence, timestamp):
        ...
        try:
            ...
            if self.is_test_video:
                ...
                with transaction.atomic():
                    alert = Alert.objects.create(
                        title="TEST Fire/Smoke Detection (Video File)",
                        description=f"Test video detection of fire/smoke with {confidence:.2f} confidence. Video: {os.path.basename(self.video_file)}",
                    ...
                    print(f"[INFO] Alert {alert.id} created successfully for test video fire/smoke detection")
                    return alert
            ...
            if hasattr(self.camera.user, 'role') and self.camera.user.role == 'admin':
                title = "TEST Fire/Smoke Detection"
                description = f"Test detection of fire/smoke with {confidence:.2f} confidence. Video clip recorded with detection highlights."
            else:
                title = "Fire/Smoke Detection Alert"
                description = f"Detected fire/smoke on camera {self.camera.name} with {confidence:.2f} confidence. Video clip recorded for review."
            ...
            with transaction.atomic():
                ...
                print(f"[INFO] Alert {alert.id} created successfully for fire/smoke detection on camera {self.camera.id}")
                return alert
    ...
    def cleanup(self):
        ...
        try:
            ...
            if self.detection_active and self.detection_frames:
                ...
                self.save_clip(thumbnail_frame, self.detection_frames, time.time(), 0.98)
            ...
            if self.cuda_context:
                try:
                    ...
                    pass
            print(f"[INFO] FireSmokeDetector cleanup completed for thread {threading.current_thread().name}")
        except Exception as e:
