...
class ChokingDetector:
        ...
        if self.is_test_video:
            ...
            try:
                self.camera, created = Camera.objects.get_or_create(
                    name="TEST VIDEO CAMERA - Choking Detection",
        ...
        try:
            ...
            if label == self.choking_label and confidence >= 0.98:
                if not self.detection_active:
                    ...
                    print(f"[INFO] Choking detection started with confidence: {confidence:.2f}")
                ...
                if self.detection_active:
                    print(f"[INFO] Choking detection ended")
                    self.detection_active = False
                    self.detection_frames = []
                    self.first_detection_frame = None
    ...
    def save_clip_on_detection(self, current_time, confidence):
        """Save video clip when choking is detected"""
        # Check cooldowns
        ...
        if not frames_to_save:
            print("[WARNING] No frames to save for choking detection")
            return
        ...
        try:
            ...
            print(f"[ERROR] Failed to save choking detection clip: {e}")
            ...
            if self.is_test_video:
                ...
                with transaction.atomic():
                    alert = Alert.objects.create(
                        title="TEST Choking Detection (Video File)",
                        description=f"Test video detection of choking with {confidence:.2f} confidence. Video: {os.path.basename(self.video_file)}",
                        alert_type='choking',
                    ...
                    print(f"[INFO] Alert {alert.id} created successfully for test video choking detection")
                    return alert
