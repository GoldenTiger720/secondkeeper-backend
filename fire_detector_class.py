...
class FireDetector:
    def __init__(self, rtsp_url, video_file, use_rtsp=False):
        ...
        self.output_dir = "detection_clips"
        self.engine_path = "fire.engine"
        self.label_binarizer_path = "./model/fire.pickle"
        self.image_size = 128
    ...
    def _initialize_components(self):
        ...
        try:
            ...
            print(f"[INFO] FireDetector initialized successfully in process {os.getpid()}")
            
        except Exception as e:
            print(f"[ERROR] Failed to initialize FireDetector: {e}")
            print(f"[ERROR] This might be due to:")
            print(f"[ERROR] - NVIDIA GPU not available or not properly configured")
            print(f"[ERROR] - CUDA drivers not installed or incompatible")
            print(f"[ERROR] - GPU being used by another process")
            print(f"[ERROR] - Permission issues")
            print(f"[ERROR] - TensorRT engine file not found or corrupted")
            import traceback
            traceback.print_exc()
            raise
    ...
    def process_frame(self, frame, fps):
        ...
        try:
            ...
            if self.host_input is None or self.lb is None:
                print("[ERROR] FireDetector not properly initialized")
                return frame
    ...
    def save_clip(self, frame, current_time):
        ...
        try:
            if not self.thumbnail_saved:
                ...
                thumbnail_path = os.path.join(self.output_dir, f"fire_thumbnail_{timestamp_str}.jpg")
            ...
            if current_time - self.last_clip_saved_time > self.cooldown_seconds:
                ...
                os.remove(output_path)  # Remove the original file if needed
    ...
    def cleanup(self):
        ...
        try:
            ...
            print(f"[INFO] FireDetector cleanup completed for process {os.getpid()}")
        except Exception as e:
